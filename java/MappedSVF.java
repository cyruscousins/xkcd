...
public class MappedSVF extends SVF{
	...
	public MappedSVF(float[] data, int length, float scale){
		this.data = data;
		this.length = length;
		l1 = length + 1;
		this.scale = scale;
	}
	public static MappedSVF translateToSVF(float[] data, int length, float scale){
		int l1 = length + 1;
		...
		return new MappedSVF(ndata, length, scale);
	}
	public static MappedSVF translateToSVF(float[][] data, int length, float scale){
		int l1 = length + 1;
		float[] ndata = new float[l1 * l1];
